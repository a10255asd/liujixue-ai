...
3. 关系校验：`projectRefs`、`relatedQuestions`、`references` 等引用必须存在；路线阶段的可选 `knowledgeRefs` / `questionRefs` 必须指向已发布的知识点和面试题。
...
数据：按 `order` 升序的全部路线阶段（含第 0 阶段零基础先导，共 8 个）。  
主要组件：`RoadmapRail`、`StageSection`、`OutputChecklist`、`StageRefs`。  
交互：移动端按阶段纵向浏览；桌面端可使用吸顶阶段导航。  
引用渲染：阶段携带可选 `knowledgeRefs` / `questionRefs` 时，在目标与产出下方渲染"知识点学习顺序"（有序列表，链接到 `/knowledge/[slug]`，显示知识点标题）和"自测面试题"（链接到 `/interview/[id]`，显示题干）；未携带这两个字段的阶段不渲染该区块。桌面两列、960px 以下单列，390px 不横向溢出。  
错误处理：存在重复 order、断裂 prerequisite 或指向不存在/未发布内容的引用时构建失败。
...
### 8.11 `/labs/rag-retrieval`

确定性 RAG 原型。数据位于 `content/labs/rag-documents.json` 和 `content/labs/rag-evaluation.json`，核心引擎位于 `lib/labs/rag-retrieval.ts`。

摄取以文档章节为稳定块，保留 `documentId`、版本、标题和位置。关键词基线使用字符词面重叠；混合模式加入语料稀有词权重与显式概念标签。回答只抽取命中章节并附引用，不调用向量服务或生成模型。

评估固定输出 Hit@3、MRR、引用覆盖和未知问题拒答率。浏览器输入只在本地计算，不保存、不发请求；该原型验证检索与评估契约，不宣称具备生产向量检索、租户权限或模型忠实度评估。
...
服务端运行时固定最多 4 个工具步，开放 `search_knowledge`、`inspect_project_evidence` 两个真实只读工具，以及 `save_learning_note` 一个受审批写工具。参数均使用严格 Schema；读取使用 `knowledge:read` 与 `projects:read`，写入使用 `notes:write`，工具输出来自当前内容与运行仓储，不从浏览器伪造。
...
运行入口已增加固定窗口限流。配置 Upstash/Vercel Redis 时使用原子 `EVAL` 计数，并将完成运行保存 24 小时；`GET /api/agent/run?id=<runId>` 只允许在当前签名会话命名空间内回放单条记录，不提供列表。运行、检查点和笔记 Redis key 均包含 actor 哈希，知道其他会话的 `runId` 也不能读取。未配置 Redis 时，夹具模式使用进程内基础限流，生产不保存；真实模型模式则直接返回 `503`。

客户端在创建运行前生成 UUID 并随 `runId` 发送。运行器每轮只允许一个工具调用，在工具结果确认后保存 `RuntimeCheckpoint v2`；`POST { resumeRunId }` 使用原 run id、历史、观察、Trace、Token 和下一轮序号恢复，避免重复执行已完成工具。写工具在副作用前保存 `waiting_approval` 与完整待审批调用，`POST { approvalRunId, decision }` 只能由同一签名会话批准或拒绝；批准使用 `runId + callId` 作为幂等键。20 条基线评测统一输出精确工具序列、完成率、P95 延迟、Token 和 OpenAI request id。
...
安全回归固定覆盖正常完成、临时故障恢复、越权拦截、步数预算终止和人工审批五类轨迹；另有 20 条任务契约与统一基线报告。签名会话与租户隔离已通过本地回归，但当前缺少生产 Redis 跨实例恢复、生产身份冒烟、真实模型报告和 Token 成本基线，因此项目状态保持 `prototype`。

### 8.13 `/labs/agent-evaluation`

确定性 Agent 评测与发布门禁原型。评测引擎位于 `lib/labs/agent-evaluation.ts`，交互页面位于 `components/labs/agent-evaluation-lab.tsx`。

同一组五类 Agent 目标分别以宽松循环和受控状态机运行事实接受评测，固定维度为结果、轨迹、权限、预算和副作用。报告输出样本分、综合分、首个回归位置和关键违规；发布门禁同时检查综合分、单样本最低分和关键安全违规，后者不能被均分抵消。

当前 V1 事实为仓库内稳定回归夹具，V2 直接由 `controlled-agent.ts` 的轨迹复算。该页不连接真实模型、数据库或外部工具，也不提供真实延迟和成本结论。
