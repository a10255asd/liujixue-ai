...
状态：Batch 2 已完成，下一主线进入 Batch 4 求职强化。Batch 3 详情页已在 Batch 1 提前完成。
...
1. 新增 Agent 工程师岗位能力矩阵，把常见 JD 要求映射到路线、知识、题目和项目证据。
2. 新增入门自测，只做确定性评分和学习路径推荐，不接模型 API。
3. 给 6 个项目补齐测试策略、部署方案、验收清单和 3 分钟项目讲解模板。
4. 增加按 8 周执行的学习计划，每周绑定明确产出和面试复盘。
5. 继续抽检题目答案与官方来源，不为增加数量继续扩题。
