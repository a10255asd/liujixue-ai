...
1. 采集有来源和日期的真实 Agent 工程师 JD 样本，校准现有 8 个能力域。
2. 把 4 个原型证据映射为已有证据、缺口和下一行动，保持成熟度表达诚实。
3. 复用现有题目与项目证据生成固定模拟面试会话和可重复评分。
4. 完成薄弱项定位、下一轮训练建议、单测和桌面/390px 验收。
5. 本阶段不增加新项目数量，不接账号、数据库或模型自动评判。
