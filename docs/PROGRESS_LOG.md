...
状态：Batch 7 阶段 1、2 已完成。Prompt 回归与可评估 RAG 两个原型均可运行，6 个项目当前为 2 个 `prototype`、4 个 `blueprint`、0 个 `verified`；线上基础设施继续沿用已接通的 GitHub、Vercel 和 `ai.liujixue.cn`。
...
把 `task-planning-agent` 从 `blueprint` 提升为 `prototype`：

1. 使用确定性状态机跑通计划、工具调用、观察、重试和终止。
2. 覆盖成功、可重试失败、不可重试失败、超步数和人工审批五类轨迹。
3. 每条轨迹可回放，并明确幂等键、权限与恢复边界。
4. 提供交互页面、单元测试、失败注入和桌面/390px 验收后再修改成熟度。
5. Agent 原型完成前不新增项目、题目、账号、数据库或真实外部工具。
