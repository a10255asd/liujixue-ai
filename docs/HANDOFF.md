...
当前已完成 Batch 2：设计与技术骨架、全部核心页面和详情页、33 个知识点、80 道面试题、6 个项目、14 个官方资料入口、内容审核契约、URL 筛选和响应式验收。
...
优先执行 `docs/IMPLEMENTATION_PLAN.md` 的 Batch 4：

1. 先阅读 `docs/PROGRESS_LOG.md` 的“下一步唯一主线”。
2. 增加 Agent 工程师岗位能力矩阵和自测页。
3. 为 6 个项目补充测试策略、部署边界和项目讲解模板。
4. 将路线阶段与知识点、题目、项目形成可执行周计划。
5. 不增加登录、收藏、模型调用或数据库。
6. 本地完整验证通过后更新进度日志。
