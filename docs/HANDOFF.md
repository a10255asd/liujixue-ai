...
当前已完成 Batch 6：设计与技术骨架、全部核心页面和详情页、33 个知识点、80 道面试题、6 个项目、14 个官方资料入口、8 个求职能力域，以及 3 条包含 7 个交付任务的核心训练路径均已通过正式域名上线。
...
1. 优先审计训练路径引用的 4 个核心项目，不继续堆内容数量。
2. 明确区分项目模板和真实可运行仓库，缺少实现就先做最小闭环。
3. 为核心项目补仓库、启动、测试、部署、验收和 3 分钟讲解证据。
4. 完成一个 RAG 项目和一个 Agent 项目后，再做真实 JD 校准和模拟面试。
5. 不立即增加登录、收藏、模型调用、数据库或更多题目。
