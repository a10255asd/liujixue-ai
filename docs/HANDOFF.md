...
当前已完成 Batch 7 阶段 1：在 Batch 6 的内容与训练基线上完成项目证据审计，并交付首个可运行的 Prompt 回归原型。6 个项目目前为 1 个 `prototype`、5 个 `blueprint`、0 个 `verified`，页面不会再把方案模板写成已完成项目。
...
2. 用仓库内确定性文档集完成摄取、切分、检索、引用返回闭环。
3. 建立至少 10 条检索评估样例，并输出可复现的检索指标。
4. 补运行入口、单测、失败样例和双端验收后，才能把 `rag-knowledge-base` 改为 `prototype`。
5. RAG 与一个受控 Agent 达到原型门禁后，再做真实 JD 校准和模拟面试。
...
- 新增固定夹具 Prompt 回归原型 `/labs/prompt-regression`，当前项目状态为 1 个原型、5 个模板、0 个已验证交付。
- 最终验证：内容校验、类型检查、Lint、15 项单元测试、135 页生产构建通过；Playwright 33 项通过、1 项跳过。
- 下一条唯一主线：按 `PROJECT_EVIDENCE_AUDIT.md` 实现 RAG 企业知识库原型，不增加内容数量。
