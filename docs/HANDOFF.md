...
当前已完成 Batch 7 阶段 1、2、3、4：完成项目证据审计，并交付 Prompt 回归、可评估 RAG、受控 Agent 与 Agent 评测控制台四个原型。6 个项目目前为 4 个 `prototype`、2 个 `blueprint`、0 个 `verified`，页面不会把方案模板写成已完成项目。
...
下一批只做真实 JD 能力校准与模拟面试闭环：

1. 阅读 `PROJECT_EVIDENCE_AUDIT.md`，不要继续增加项目数量。
2. 收集有来源、日期和岗位上下文的真实 Agent 工程师 JD 样本，校准现有 8 个能力域。
3. 把 4 个原型证据映射为岗位已有证据、缺口与下一行动，不把原型写成生产交付。
4. 复用现有面试题和项目证据生成固定模拟面试会话、评分与薄弱项定位。
5. 完成单测和双端验收后，再评估现有原型的 `verified` 升级条件。
...
- 下一条唯一主线：按 `PROJECT_EVIDENCE_AUDIT.md` 做真实 JD 能力校准与模拟面试闭环，不增加项目数量。
